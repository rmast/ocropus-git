...
namespace ocropus {
    ...
    int TextImageSegByLogReg::getColor(floatarray &prob_map, int index){
        ...
        colib::byte red = colib::byte(255*(max(prob_map(index,text),
                                 prob_map(index,table))));

        colib::byte green = colib::byte(255*(max(prob_map(index,halftone),
                                   max(prob_map(index,logo),
                                       prob_map(index,noise)))));

        colib::byte blue = colib::byte(255*(max(prob_map(index,drawing),
